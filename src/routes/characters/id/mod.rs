...
#[template(path = "characters/id/index.html")]
struct Index {
...
async fn index_get(
  ...
  let (
    ...
    get_character(state, &req, &session, vampire_id),
...
async fn index_post(
  ...
  let _character = get_character(state, &req, &session, vampire_id).await?;
...
WHERE humanity_xp_cost.change_type = CASE WHEN $2::INT > 0 THEN 'gain' ELSE 'loss' END
  ...
  tx.commit().await?;
  see_other(&format!("/characters/{vampire_id}/?saved=1"))
}
pub async fn route(
  state: &'static State,
  req: Request,
  ...
  match path_vec.pop().as_deref() {
    ...
    Some("") => {
      ...
      match req.method() {
        &Method::GET => index_get(state, req, session, vampire_id).await,
        &Method::POST => index_post(state, req, session, vampire_id).await,
